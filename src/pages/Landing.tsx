...
const tweets = [
  {
    name: "Thiruman", handle: "@piratethiru", time: "2m",
    text: "Just dumped 3 weeks of chaotic product notes into @DumpStashAI — got a full PRD, roadmap + theme clusters back in 30s. This is the future 🤯🔥",
    reply: "47", rt: "312", like: "2.1k",
  },
  {
    name: "Priya Sharma", handle: "@priya_builds", time: "14m",
    text: "been using @DumpStashAI for a week. I type messy voice-to-text thoughts, it spits out structured ideas + PRDs. life changed 💀",
    reply: "31", rt: "198", like: "1.4k",
  }
];

const slackMessages = [
  {
    workspace: "Product Team", channel: "ideas",
    user: "Priya M.", text: "What if we auto-generate the PRD straight from the brain dump? 🤔",
  }
];
...
const NeuralFlowCanvas = () => {
  ...
  useEffect(() => {
    ...
    const leftNodes = [{ x: 0.1, y: 0.2 }, { x: 0.1, y: 0.5 }, { x: 0.1, y: 0.8 }];
    const center = { x: 0.5, y: 0.5 };
    const rightNodes = [{ x: 0.9, y: 0.25 }, { x: 0.9, y: 0.5 }, { x: 0.9, y: 0.75 }];
    ...
    const makePath = (from: { x: number; y: number }, to: { x: number; y: number }) => {
      const mx = (from.x + to.x) / 2;
      return { sx: from.x, sy: from.y, cp1x: mx - 0.05, cp1y: from.y, cp2x: mx + 0.05, cp2y: to.y, ex: to.x, ey: to.y };
    };

    const spawnParticle = () => {
      const isLeft = Math.random() > 0.4;
      const from = isLeft ? leftNodes[Math.floor(Math.random() * leftNodes.length)] : center;
      const to = isLeft ? center : rightNodes[Math.floor(Math.random() * rightNodes.length)];
      particles.push({
        path: makePath(from, to),
        t: 0, speed: 0.004 + Math.random() * 0.003, size: 1.5, opacity: 0.5,
      });
    };
    ...
    const draw = () => {
      ...
      for (let i = particles.length - 1; i >= 0; i--) {
        ...
        const px = u * u * u * sx + 3 * u * u * t * cp1x + 3 * u * t * t * cp2x + t * t * t * ex;
        const py = u * u * u * sy + 3 * u * u * t * cp1y + 3 * u * t * t * cp2y + t * t * t * ey;
        ctx.beginPath(); ctx.arc(px * w, py * h, p.size, 0, Math.PI * 2);
        ctx.fillStyle = `hsla(142, 71%, 55%, ${Math.sin(t * Math.PI) * p.opacity})`;
        ctx.fill();
      }
      animRef.current = requestAnimationFrame(draw);
    };
    ...
    <div className="min-h-screen bg-background text-foreground overflow-hidden">
      ...
      <nav className="relative z-40 flex items-center justify-between px-6 md:px-12 py-5 max-w-7xl mx-auto">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[15px] font-bold">DumpStash</span>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="sm" onClick={() => navigate("/dashboard")}>Log in</Button>
          <Button size="sm" onClick={() => navigate("/dashboard")}>Get Started <ArrowRight className="ml-1 w-3.5" /></Button>
        </div>
      </nav>

      {/* Hero */}
      <section className="relative z-10 pt-20 md:pt-32 pb-10 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <motion.h1 
            variants={fadeUp} initial="hidden" animate="visible" custom={1}
            className="text-5xl md:text-7xl font-extrabold tracking-tight mb-6"
          >
            Dump your chaos. <br/><span className="text-muted-foreground">Ship with clarity.</span>
          </motion.h1>
          <motion.p 
            variants={fadeUp} initial="hidden" animate="visible" custom={2}
          ...
          >
            DumpStash AI turns your raw brain dumps into structured ideas, product docs,
            and interactive roadmaps.
          </motion.p>
        </div>
      </section>

      {/* Merged Neural Flow Visualization */}
      <div className="relative h-[550px] w-full max-w-7xl mx-auto mb-20">
        <NeuralFlowCanvas />
        <div className="absolute inset-0 grid grid-cols-1 md:grid-cols-3 items-center z-10 px-6">
          
          {/* Left: Chaos Input (Social Cards) */}
          <div className="flex flex-col gap-4 items-start">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} className="w-64 p-3 bg-card border rounded-xl shadow-xl">
              <div className="flex gap-2 mb-2"><div className="w-6 h-6 rounded-full bg-blue-500" /><span className="text-[10px] font-bold">@piratethiru</span></div>
              <p className="text-[11px] leading-tight">Just dumped 3 weeks of notes... DumpStash is insane. 🤯</p>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="w-64 p-3 bg-[#1a1d21] border border-[#2d2d2d] rounded-xl shadow-xl">
              <div className="flex gap-2 mb-2"><div className="w-6 h-6 rounded bg-emerald-500" /><span className="text-[10px] text-white">#product-ideas</span></div>
              <p className="text-[11px] text-gray-300">What if we auto-gen the PRD from the dump? 🤔</p>
            </motion.div>
          </div>

          {/* Center: AI Hub */}
          <div className="flex justify-center">
            <motion.div initial={{ scale: 0.8, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} className="p-8 rounded-3xl border bg-card/90 backdrop-blur-md shadow-2xl text-center">
              <Zap className="w-12 h-12 mx-auto mb-2 text-emerald-500" />
              <p className="font-bold">DumpStash AI</p>
              <p className="text-[10px] font-mono text-muted-foreground">STRUCTURING...</p>
            </motion.div>
          </div>

          {/* Right: Clarity Output */}
          <div className="flex flex-col gap-4 items-end">
            {outputItems.map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.1 }} className="w-56 p-3 bg-card border rounded-xl flex items-center gap-3">
                <item.icon className="w-4 h-4 text-emerald-500" />
                <div><p className="text-[11px] font-bold">{item.label}</p><p className="text-[9px] text-muted-foreground">{item.sub}</p></div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section id="features" className="relative z-10 py-24 px-6 border-t border-border/50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-bold tracking-tight">Everything you need to <span className="text-emerald-500">think clearly</span></h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {features.map((f, i) => (
              <motion.div
                key={f.title}
                variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}
                className="p-8 rounded-2xl border bg-card/50 hover:bg-card transition-colors"
              >
                <div className="w-10 h-10 rounded-xl bg-foreground/5 border flex items-center justify-center mb-5">
      ...
      <section className="relative z-10 py-24 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <div className="p-12 md:p-20 rounded-[3rem] border bg-card/30 backdrop-blur-sm relative overflow-hidden">
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-emerald-500/5 blur-[120px] pointer-events-none" />
            <h2 className="text-4xl md:text-5xl font-bold mb-6">Ready to dump smarter?</h2>
            <p className="text-muted-foreground mb-10 max-w-lg mx-auto text-lg">
              Stop losing ideas in scattered notes. Let AI turn your chaos into clarity.
            </p>
            <Button size="lg" className="h-14 px-10 text-lg rounded-2xl" onClick={() => navigate("/dashboard")}>
      ...
      <footer className="border-t border-border py-12 px-6">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold">DumpStash AI</span>
          </div>
          ...
          <p className="text-xs font-mono text-muted-foreground">© 2026 DumpStash AI. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
